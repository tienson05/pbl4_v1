    ...
    {
        setupUI();
        // --- (QUAN TRỌNG) BẬT MENU CHUỘT PHẢI ---
        packetList->setContextMenuPolicy(Qt::CustomContextMenu);
        connect(packetList, &QTableWidget::customContextMenuRequested,
                this, &PacketTable::showContextMenu);
    ...
    {
        // 1. Lấy item tại vị trí chuột
        QTableWidgetItem *item = packetList->itemAt(pos);
        if (!item) return;

        // 2. Lấy dữ liệu gói tin từ cột ẩn (Cột 7) của dòng đó
        int row = item->row();
        QTableWidgetItem *hidden = packetList->item(row, 7);
        if (!hidden) return;

        // Trích xuất PacketData
        PacketData packet = hidden->data(Qt::UserRole).value<PacketData>();

        // 3. Kiểm tra xem có stream_index không (-1 là không có)
        if (packet.stream_index < 0) return;

        // 4. Tạo Menu
        QMenu contextMenu(this);

        // Tạo action "Follow Stream"
        QString streamName = (packet.app.protocol == "QUIC") ? "QUIC" : "TCP/UDP";
        QString actionText = QString("Follow %1 Stream (Stream #%2)").arg(streamName).arg(packet.stream_index);

        QAction *actionFollow = contextMenu.addAction(actionText);

        // 5. Khi bấm Menu -> Bắn tín hiệu ra ngoài
        connect(actionFollow, &QAction::triggered, [this, packet]() {
            // Tạo chuỗi lọc: "stream == 5"
            QString filter = QString("stream == %1").arg(packet.stream_index);

            // Hét lên cho MainWindow nghe thấy
            emit filterRequested(filter);
        });

        // Hiện menu ngay tại chuột
    ...
    void PacketTable::insertPacketRow(const PacketData &packet)
    {
        ...
        QList<QTableWidgetItem*> items = {
            ...
            new QTableWidgetItem(getSourceAddress(packet)),      // 2 (Đã có tên Vendor)
            new QTableWidgetItem(getDestinationAddress(packet)), // 3 (Đã có tên Vendor)
    ...
    void PacketTable::showPacketDetails(const PacketData &packet)
    {
        ...
        // Phân giải Vendor + Hiển thị đầy đủ: "Vmware_C0:00:08 (00:50:56:C0:00:08)"
        ...
        if (packet.is_ipv4) {
            ...
            if (!flags_str.contains(QChar(','))) flags_str += "No Flags"; else flags_str.chop(2);
            flags_str += ")";
            ...
            // Phân giải MAC trong ARP
            QString senderMacRaw = macToString(packet.arp.sender_mac);
            QString targetMacRaw = macToString(packet.arp.target_mac);

            addField(arp, "Sender MAC", QString("%1 (%2)").arg(getResolvedMacLabel(senderMacRaw)).arg(senderMacRaw), l3_offset + 8, 6);
            addField(arp, "Sender IP", ipToString(packet.arp.sender_ip), l3_offset + 14, 4);
            addField(arp, "Target MAC", QString("%1 (%2)").arg(getResolvedMacLabel(targetMacRaw)).arg(targetMacRaw), l3_offset + 18, 6);
    ...
    {
        if (p.is_arp) return getResolvedMacLabel(macToString(p.eth.src_mac)); // Resolve cho ARP
        if (p.is_ipv4) return ipToString(p.ipv4.src_ip);
        if (p.is_ipv6) return ipv6ToString(p.ipv6.src_ip);
        return getResolvedMacLabel(macToString(p.eth.src_mac)); // Resolve cho non-IP
    }

    QString PacketTable::getDestinationAddress(const PacketData &p)
    {
        if(p.is_arp) return getResolvedMacLabel(macToString(p.eth.dest_mac)); // Resolve cho ARP
        if (p.is_ipv4) return ipToString(p.ipv4.dest_ip);
        if (p.is_ipv6) return ipv6ToString(p.ipv6.dest_ip);
        return getResolvedMacLabel(macToString(p.eth.dest_mac)); // Resolve cho non-IP
    }
